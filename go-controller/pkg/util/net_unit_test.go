...
func TestIPAddrToHWAddr(t *testing.T) {
	tests := []struct {
		...
		// TODO: The function may need to be modified to ensure that the input net.IP passed is not empty.
		// Passing empty net.IP causes panic
		/*{
			desc:	"test empty instance of net.IP",
			inpIP: net.IP{},
		},*/
		{
	...
	for i, tc := range tests {
		t.Run(fmt.Sprintf("%d:%s", i, tc.desc), func(t *testing.T) {
			...
			t.Log(res)
			assert.Equal(t, res[0], byte(0x0A))
			assert.Equal(t, res[1], byte(0x58))
		})
	}
}
